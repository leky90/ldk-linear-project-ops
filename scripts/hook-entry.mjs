...
export function handle(name, payload, binding) {
  ...
  if (name === "UserPromptSubmit") {
    ...
    if (/(?:báo cáo|tổng quan|tiến độ|project status|status report|project update|cập nhật.*health|on track|at risk|off track)/iu.test(prompt)) return wrap(attributes, "Use $linear-project-status. Reports are read-only; publish a native Linear Project Update only on a direct publish/update request and validate its evidence-based health first.");
  ...
  if (name === "PreToolUse" && isMutation(toolName)) return wrap(attributes, "Before mutating Linear, verify exact project/team IDs, native hierarchy, planning properties, current role/state/DoR/DoD, resources and relations. For deletion, require an exact approved entity ID and a verified canonical destination when preserving content. Do not publish machine telemetry or raw JSON.");
  if (name === "PostToolUse" && isMutation(toolName)) return wrap(attributes, "Re-read each affected Linear entity. Confirm native hierarchy, project/milestone properties, resources, relations, human handoff or native Project Update match the actual result; report every skipped, conflicted, failed, and deleted entity.");
