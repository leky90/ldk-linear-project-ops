...
description: Repair inconsistent Linear issues, hierarchy, handoffs, relations, states, local locks, or legacy workflow data. Use for stale work, interrupted sessions, status/evidence mismatches, abandoned locks, issue-level initiative migration, project-wide legacy audit, approved merge, or selective purge.
---
...
6. Preview ambiguous, bulk, reopen, cancel, merge, or destructive changes. Validate an approved apply plan with `validate-legacy-cleanup.mjs --apply`; every destructive entry must be individually approved.
