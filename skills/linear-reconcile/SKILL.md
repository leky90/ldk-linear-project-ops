...
7. Apply [delivery-lifecycle.md](../../references/delivery-lifecycle.md). Move a reviewed action-mode issue to `Ready to Deliver`, or keep `In Review` with a persisted delivery phase when the custom status is unavailable. Return invalidated delivery evidence to the correct active state. For a legacy `Done` issue with durable terminal proof, preserve it and record an inferred delivery mode instead of reopening it only for missing metadata.
8. Before deletion, transfer preserved content, resources, parent/child links, milestones, blockers, related links, and durable evidence to the canonical object. For a resolved blocker, re-evaluate DoR, remove `blockedBy`, optionally preserve useful history as `relatedTo`, and move to `Ready` only when DoR passes. If planned prerequisite work is missing, produce an exact create-work preview and create it only when the current request authorizes creation. Delete only exact approved IDs. Never delete real completed delivery history merely because it uses legacy metadata.
9. Write at most one human reconciliation comment when a surviving issue needs human context; never add a cleanup comment to an entity being deleted.
10. Re-read changed entities and report created, normalized, merged, converted, deleted, skipped, conflicted, failed, and whether surviving work is ready for `$linear-do-issue`.

Use [work-model.md](../../references/work-model.md), [delivery-lifecycle.md](../../references/delivery-lifecycle.md), [issue-relations.md](../../references/issue-relations.md), [comment-policy.md](../../references/comment-policy.md), and [legacy-compatibility.md](../../references/legacy-compatibility.md).
