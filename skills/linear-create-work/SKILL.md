...
6. Draft a schema-v2 work plan matching `schemas/work-plan.schema.json`, including project properties, native initiatives, milestones, resources, role-owned issues, scheduling properties, and relations that are actually known. New plans use `projectStatus: { id, name, category }` from live Linear and may declare `lifecycle.mode` plus completion criteria; `project.status` is legacy read compatibility only. Persist lifecycle mode/criteria in the Project description or an approved lifecycle resource because they are RoleFlow contract metadata, not fabricated native Linear fields.
...
11. Creating planned work does not itself prove execution began. Do not downgrade an active Project or infer Completed from a fully Done plan; use [project-lifecycle.md](../../references/project-lifecycle.md).
